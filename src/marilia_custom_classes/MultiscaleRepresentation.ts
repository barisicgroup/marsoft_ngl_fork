    ...
    create() {
        ...
        switch (this.currentScale) {
            case 0:
                ...
                const posArray = new Float32Array(elementCenterPositions.length * 3);
                const normArray = new Float32Array(elementCenterPositions.length * 3);
                const binormArray = new Float32Array(elementCenterPositions.length * 3);
                const tangArray = new Float32Array(elementCenterPositions.length * 3);
                const colArray = new Float32Array(elementCenterPositions.length * 3);
                const sizeArray = new Float32Array(elementCenterPositions.length);

                colArray.fill(1);

                for (let i = 0; i < elementCenterPositions.length; ++i) {
                    posArray[3 * i] = elementCenterPositions[i].x;
                    posArray[3 * i + 1] = elementCenterPositions[i].y;
                    posArray[3 * i + 2] = elementCenterPositions[i].z;

                    const dir = i === elementCenterPositions.length - 1 ? new Vector3(1, 0, 0) : elementCenterPositions[i + 1].clone().sub(elementCenterPositions[i]).normalize();
                    ...
                    if (i % 2 == 0) {
                        colArray[i] = i / elementCenterPositions.length;
                    }
                }
            ...
            case 1:
                ...
                for (let i = 0; i < elementCenterPositions.length; ++i) {
                    this.currentShape.addSphere(elementCenterPositions[i], [1, i / elementCenterPositions.length, 1], this.structure.elementDiamater * 0.25, "Element_" + i.toString());
                }
                break;
            default:
                ...
                for (let i = 0; i < elementRowPositions.length; ++i) {
                    this.currentShape.addCylinder(elementRowPositions[i],
                        ...
                        [1, i / elementRowPositions.length, 1], this.structure.elementDiamater * 0.5, "Element_row_" + i.toString());
                }
                break;
        }
