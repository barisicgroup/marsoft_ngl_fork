...
export function appendStructures(parentStructure: Structure, ...structuresToAppend: Structure[]) {
  ...
  structuresToAppend.forEach(structure => {
    structure.eachAtom(a => {
      atomStore.growIfFull();
      atomStore.atomTypeId[idx] = atomMap.add(a.atomname, a.element);
      ...
      /*
      // TODO Possible risk of incorrect data when
      // the chain/residue name/id/no matches the existing one?
      sb.addAtom(
        a.modelIndex + modelCount,
        a.chainname,
        a.chainid,
